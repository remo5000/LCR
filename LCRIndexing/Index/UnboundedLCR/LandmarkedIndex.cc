...
void LandmarkedIndex::buildIndex(int continueCode)
{
    ...
    if( continueCode == 0 || continueCode == 1 )
    {
        ...
        for(int x = 0; x < N; x++)
        {
            ...
            if( (getCurrentTimeInMilliSec()-constStartTime + landmarkTime) >= TIMEOUT )
            {
                cout << this->name << "::buildIndex (landmarks) times out" << endl;
                I = i;
                didNotComplete = true;
                break;
            }
            ...
            if( ((cc+1)%quotum) == 0 && cc > 0 )
            {
                double perc = cc;
                perc /= (noOfLandmarks);
                perc *= 100.0;

                cout << this->name << "::buildIndex (landmarks) cc=" << cc << " " << perc << "% , " << ",roundNo=" << a << ",size(MB)=" << size/1000
                    << ",time(s)=" << (getCurrentTimeInMilliSec()-constStartTime) << endl;
            }
        ...
        cout << this->name << "::buildIndex built all landmarks size(MB)=" << size/1000 << " ,time(s)=" << landmarkTime << endl;
    ...
    else
    {
        ...
        if( budgetOthers > 0 && doIndexOthers == true )
        {
            for(int x = 0; x < N; x++)
            {
                ...
                if( getCurrentTimeInMilliSec()-constStartTime + landmarkTime >= TIMEOUT )
                {
                    cout << this->name << "LandmarkedIndex::buildIndex (others) times out" << endl;
                    I = i;
                    break;
                }
                ...
                if( ((cc+1)%quotum) == 0 && cc > 0 )
                {
                    ...
                    cout << this->name << "::buildIndex (others) (o) cc=" << cc << " " << perc << "% , " << ",roundNo=" << a << ",size(MB)=" << size/1000
                        << ",time(s)=" << (getCurrentTimeInMilliSec()-constStartTime) << endl;
                }
            }
        }
...
bool LandmarkedIndex::queryShell(VertexID source, VertexID target, LabelSet ls)
{
    ...
    //double p = max(computeQueryFilterProbability(ls),0.1);
    //int freq = 3 / p;
